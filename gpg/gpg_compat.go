...
func publicKeyringFormat(w io.Writer, keys []types.GetKeyInfoResult) {
	for _, k := range keys {
		...
		for _, e := range entities {
			...
			if primaryIdentity != nil &&
				primaryIdentity.SelfSignature != nil &&
				primaryIdentity.SelfSignature.KeyLifetimeSecs != nil {
				sig := primaryIdentity.SelfSignature

				expiration = fmt.Sprintf("[expires: %s]",
					sig.CreationTime.Add(time.Duration(*sig.KeyLifetimeSecs)*time.Second).Format(dateFormat))
			}
